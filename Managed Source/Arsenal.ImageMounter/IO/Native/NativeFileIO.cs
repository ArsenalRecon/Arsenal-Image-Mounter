...
public static class NativeFileIO
{
    ...
    public static class UnsafeNativeMethods
    {
        ...
        [DllImport("kernel32", SetLastError = true)]
        internal static extern bool DeviceIoControl(SafeFileHandle hDevice, uint dwIoControlCode, in byte lpInBuffer, uint nInBufferSize, out byte lpOutBuffer, uint nOutBufferSize, out uint lpBytesReturned, IntPtr lpOverlapped);
        ...
        [DllImport("setupapi", SetLastError = true, CharSet = CharSet.Unicode)]
        internal static extern bool SetupDiGetINFClassW(in char InfPath, out Guid ClassGuid, out char ClassName, uint ClassNameSize, out uint RequiredSize);
    ...
    public static class PhDiskMntCtl
    {
        ...
        public static Span<byte> SendSrbIoControl(SafeFileHandle adapter, uint ctrlcode, uint timeout, Span<byte> databytes, out uint returncode)
        {

            var bufferSize = 28 + databytes.Length;
            ...
            try
            {
                ...
                {
                    var argvalue1 = 0U;
                    MemoryMarshal.Write(indata.Slice(24), ref argvalue1);
                }
                else
                {
                    var argvalue2 = databytes.Length;
                    MemoryMarshal.Write(indata.Slice(24), ref argvalue2);
                    databytes.CopyTo(indata.Slice(28));
                }

                var Response = DeviceIoControl(adapter, NativeConstants.IOCTL_SCSI_MINIPORT, indata, 0);

                returncode = MemoryMarshal.Read<uint>(Response.Slice(20));

                if (!databytes.IsEmpty)
                {
                    var ResponseLength = Math.Min(Math.Min(MemoryMarshal.Read<int>(Response.Slice(24)), Response.Length - 28), databytes.Length);
    ...
    public static string SystemArchitecture { get; private set; } = GetSystemArchitecture();

    public static string ProcessArchitecture { get; private set; } = GetProcessArchitecture();

    private static string GetSystemArchitecture()
    {

        var SystemArchitecture = RuntimeInformation.OSArchitecture.ToString();
        if (SystemArchitecture.Equals("X64", StringComparison.OrdinalIgnoreCase))
        {
    ...
    private static string GetProcessArchitecture()
    {
        ...
        if (ProcessArchitecture.Equals("X64", StringComparison.OrdinalIgnoreCase))
        {
    ...
    public static bool OfflineDiskVolumes(string device_path, bool force)
        => OfflineDiskVolumes(device_path.AsMemory(), force, CancellationToken.None);

    public static bool OfflineDiskVolumes(ReadOnlyMemory<char> device_path, bool force)
    ...
    public static bool OfflineDiskVolumes(string device_path, bool force, CancellationToken cancel)
        => OfflineDiskVolumes(device_path.AsMemory(), force, cancel);

    public static bool OfflineDiskVolumes(ReadOnlyMemory<char> device_path, bool force, CancellationToken cancel)
    {
        ...
        foreach (var volume in EnumerateDiskVolumes(device_path))
        {
            ...
            try
            {
                using var device = new DiskDevice(volume.AsMemory().TrimEnd('\\'), FileAccess.ReadWrite);
            ...
            catch (Exception ex)
            {
                ...
                if (!force)
                {
                    var dev_paths = QueryDosDevice(volume.AsMemory(4, 44)).ToArray();
...
{in_use_apps[0]}", ex);
    ...
    public static async Task<bool> OfflineDiskVolumesAsync(ReadOnlyMemory<char> device_path, bool force, CancellationToken cancel)
    {
...
{in_use_apps[0]}", ex);
    ...
    public static async Task WaitForDiskIoIdleAsync(ReadOnlyMemory<char> device_path, int iterations, TimeSpan waitTime, CancellationToken cancel)
    {

        var volumes = EnumerateDiskVolumes(device_path).ToArray();

        var dev_paths = volumes.SelectMany(volume => QueryDosDevice(volume.AsMemory(4, 44))).ToArray();
    ...
    public static string[]? EnablePrivileges(params string[] privileges)
    {
        ...
        using (token)
        {
            ...
            {
                if (UnsafeNativeMethods.LookupPrivilegeValueW('\0', MemoryMarshal.GetReference(privilege.AsSpan()), out var luid))
                {
    ...
    public static WaitHandle CreateWaitHandle(IntPtr Handle, bool inheritable)
    {
        ...
        return !UnsafeNativeMethods.DuplicateHandle(current_process, Handle, current_process, out var new_handle, 0U, inheritable, 0x2U)
            ? throw new Win32Exception()
            : (WaitHandle)new NativeWaitHandle(new_handle);
    }
    ...
    public static SystemHandleTableEntryInformation[] GetSystemHandleTable()
    {
        ...
        do
        {
            var status = UnsafeNativeMethods.NtQuerySystemInformation(SystemInformationClass.SystemHandleInformation, buffer, (int)buffer.ByteLength, out var argpuReturnLength);
        ...
        var handlecount = buffer.Read<int>(0UL);
        var arrayoffset = IntPtr.Size;
        var array = new SystemHandleTableEntryInformation[handlecount];
        buffer.ReadArray((ulong)arrayoffset, array, 0, handlecount);
    ...
    public sealed class HandleTableEntryInformation
    {
        ...
        public SystemHandleTableEntryInformation HandleTableEntry { get; private set; }
    ...
    private static readonly ConcurrentDictionary<byte, string?> _objectTypes = new();
                                                                                                   ...
                                                                                                   string filterObjectType)
    {
        ...
        foreach (var handle in handleTable)
        {
            ...
            if (handle.ProcessId == 0 || filterObjectType is not null && _objectTypes.TryGetValue(handle.ObjectType, out object_type) && !ReferenceEquals(object_type, filterObjectType) || !processInfoList.TryGetValue(handle.ProcessId, out var processInfo))
            {
            ...
            try
            {
                ...
                object_type ??= _objectTypes.GetOrAdd(handle.ObjectType, b =>
                {
                    for (; ; )
                    {
    ...
    public static bool GetDiskFreeSpace(ReadOnlyMemory<char> lpRootPathName,
                                        out uint lpSectorsPerCluster,
                                        out uint lpBytesPerSector,
                                        out uint lpNumberOfFreeClusters,
                                        out uint lpTotalNumberOfClusters)
        => UnsafeNativeMethods.GetDiskFreeSpaceW(lpRootPathName.MakeNullTerminated(),
    ...
    public static Span<byte> DeviceIoControl(SafeFileHandle device, uint ctrlcode, Span<byte> data, uint outdatasize)
    {

        var indata = (ReadOnlySpan<byte>)data;

        var indatasize = (uint)indata.Length;

        if (outdatasize > indatasize)
        {
            data = new byte[(int)(outdatasize - 1U) + 1];
        }

        var rc = UnsafeNativeMethods.DeviceIoControl(device,
                                                     ctrlcode,
                                                     MemoryMarshal.GetReference(indata),
                                                     indatasize,
                                                     out data[0],
                                                     (uint)data.Length,
                                                     out outdatasize,
    ...
    public static SafeFileHandle CreateFile(ReadOnlyMemory<char> FileName, FileSystemRights DesiredAccess, FileShare ShareMode, IntPtr SecurityAttributes, uint CreationDisposition, int FlagsAndAttributes, IntPtr TemplateFile)
    {

        var handle = UnsafeNativeMethods.CreateFileW(FileName.MakeNullTerminated(),
                                                                  DesiredAccess,
                                                                  ShareMode,
                                                                  SecurityAttributes,
                                                                  CreationDisposition,
    ...
    public static SafeFileHandle OpenFileHandle(ReadOnlyMemory<char> FileName, FileAccess DesiredAccess, FileShare ShareMode, FileMode CreationDisposition, bool Overlapped)
    {
    ...
    public static SafeFileHandle OpenFileHandle(ReadOnlyMemory<char> FileName, FileAccess DesiredAccess, FileShare ShareMode, FileMode CreationDisposition, FileOptions Options)
    ...
    /// <param name="CreationDisposition">Open/creation mode.</param>
    /// <param name="Options">Specifies whether to request overlapped I/O.</param>
    public static SafeFileHandle OpenFileHandle(ReadOnlyMemory<char> FileName, FileAccess DesiredAccess, FileShare ShareMode, FileMode CreationDisposition, uint Options)
    {
    ...
    /// <param name="ShareMode">Share mode to request.</param>
    /// <param name="CreationDisposition">Open/creation mode.</param>
    public static SafeFileHandle OpenBackupHandle(ReadOnlyMemory<char> FilePath, FileAccess DesiredAccess, FileShare ShareMode, FileMode CreationDisposition)
    {

        if (FilePath.Span.IsWhiteSpace())
        {
    ...
    /// <param name="ShareMode">Share mode to request.</param>
    /// <param name="CreationDisposition">Open/creation mode.</param>
    public static SafeFileHandle TryOpenBackupHandle(ReadOnlyMemory<char> FilePath, FileAccess DesiredAccess, FileShare ShareMode, FileMode CreationDisposition)
    {
    ...
    /// <param name="Overlapped">Specifies whether to request overlapped I/O.</param>
    public static FileStream OpenFileStream(ReadOnlyMemory<char> FileName, FileMode CreationDisposition, FileAccess DesiredAccess, FileShare ShareMode, int BufferSize, bool Overlapped)
    ...
    /// <param name="CreationDisposition">Open/creation mode.</param>
    /// <param name="Overlapped">Specifies whether to request overlapped I/O.</param>
    public static FileStream OpenFileStream(ReadOnlyMemory<char> FileName, FileMode CreationDisposition, FileAccess DesiredAccess, FileShare ShareMode, bool Overlapped)
    ...
    /// <param name="CreationDisposition">Open/creation mode.</param>
    /// <param name="Options">Specifies whether to request overlapped I/O.</param>
    public static FileStream OpenFileStream(ReadOnlyMemory<char> FileName, FileMode CreationDisposition, FileAccess DesiredAccess, FileShare ShareMode, FileOptions Options)
    ...
    public static long GetFileSize(string Filename)
        => GetFileSize(Filename.AsMemory());

    public static long GetFileSize(ReadOnlyMemory<char> Filename)
    {
    ...
    public static bool TryGetFileAttributes(ReadOnlyMemory<char> Filename, out FileAttributes attributes)
    {
    ...
    public static long? GetDiskSize(SafeFileHandle SafeFileHandle) => UnsafeNativeMethods.DeviceIoControl(SafeFileHandle, NativeConstants.IOCTL_DISK_GET_LENGTH_INFO, IntPtr.Zero, 0U, out
        long FileSize, 8U, out var arglpBytesReturned, IntPtr.Zero)
            ? FileSize
    ...
    public static bool IsDiskWritable(SafeFileHandle SafeFileHandle)
    {
        ...
        var rc = UnsafeNativeMethods.DeviceIoControl(SafeFileHandle, NativeConstants.IOCTL_DISK_IS_WRITABLE, IntPtr.Zero, 0U, IntPtr.Zero, 0U, out var arglpBytesReturned, IntPtr.Zero);
        if (rc)
        {
            return true;
        }
    ...
    public static string GetLongFullPath(ReadOnlyMemory<char> path)
    {
    ...
    public static bool DismountVolumeFilesystem(SafeFileHandle Device, bool Force)
    {
        ...
        for (var i = 0; i <= 10; i++)
        {
            ...
            lock_result = UnsafeNativeMethods.DeviceIoControl(Device, NativeConstants.FSCTL_LOCK_VOLUME, IntPtr.Zero, 0U, IntPtr.Zero, 0U, out var arglpBytesReturned, default);
...
&& UnsafeNativeMethods.DeviceIoControl(Device, NativeConstants.FSCTL_DISMOUNT_VOLUME, IntPtr.Zero, 0U, IntPtr.Zero, 0U, out var arglpBytesReturned1, default);
    }
...
&& UnsafeNativeMethods.DeviceIoControl(Device, NativeConstants.FSCTL_DISMOUNT_VOLUME, IntPtr.Zero, 0U, IntPtr.Zero, 0U, out var arglpBytesReturned1, default);
    ...
    public static DISK_GEOMETRY? GetDiskGeometry(SafeFileHandle hDevice) => UnsafeNativeMethods.DeviceIoControl(hDevice, NativeConstants.IOCTL_DISK_GET_DRIVE_GEOMETRY, IntPtr.Zero, 0U, out
        DISK_GEOMETRY DiskGeometry, (uint)PinnedBuffer<DISK_GEOMETRY>.TypeSize, out var arglpBytesReturned, default)
            ? DiskGeometry
    ...
    /// </summary>
    /// <param name="hDevice">Handle to device.</param>
    public static SCSI_ADDRESS? GetScsiAddress(SafeFileHandle hDevice) => UnsafeNativeMethods.DeviceIoControl(hDevice, NativeConstants.IOCTL_SCSI_GET_ADDRESS, IntPtr.Zero, 0U, out
        SCSI_ADDRESS ScsiAddress, (uint)PinnedBuffer<SCSI_ADDRESS>.TypeSize, out var arglpBytesReturned, default)
            ? ScsiAddress
    ...
    public static SCSI_ADDRESS? GetScsiAddress(ReadOnlyMemory<char> Device)
    {
    ...
    public static SCSI_ADDRESS? GetScsiAddressForNtDevice(string NtDevicePath)
    {
        ...
        try
        {
            using var hDevice = NtCreateFile(NtDevicePath, 0, 0, FileShare.ReadWrite, NtCreateDisposition.Open, NtCreateOptions.NonDirectoryFile, 0, null, out var argWasCreated);
    ...
    public static bool? GetStorageTrimProperties(SafeFileHandle hDevice)
    {
        ...
        return !UnsafeNativeMethods.DeviceIoControl(hDevice, NativeConstants.IOCTL_STORAGE_QUERY_PROPERTY,
                                                                                                      StoragePropertyQuery, (uint)PinnedBuffer<STORAGE_PROPERTY_QUERY>.TypeSize,
                                                                                                      out DEVICE_TRIM_DESCRIPTOR DeviceTrimDescriptor, (uint)PinnedBuffer<DEVICE_TRIM_DESCRIPTOR>.TypeSize,
                                                                                                      out _, IntPtr.Zero)
            ? default
    ...
    /// </summary>
    /// <param name="hDevice">Handle to device.</param>
    public static STORAGE_DEVICE_NUMBER? GetStorageDeviceNumber(SafeFileHandle hDevice) => UnsafeNativeMethods.DeviceIoControl(hDevice, NativeConstants.IOCTL_STORAGE_GET_DEVICE_NUMBER, IntPtr.Zero, 0U, out
        STORAGE_DEVICE_NUMBER StorageDeviceNumber, (uint)PinnedBuffer<STORAGE_DEVICE_NUMBER>.TypeSize, out var arglpBytesReturned, default)
            ? StorageDeviceNumber
    ...
    public static string GetPhysicalDriveNameForNtDevice(string ntdevice)
    {
        ...
        using var hDevice = NtCreateFile(ntdevice, 0, 0, FileShare.ReadWrite, NtCreateDisposition.Open, 0, 0, null, out var argWasCreated);
    ...
    public static string? QueryDirectoryJunction(ReadOnlyMemory<char> source)
    {

        using var hdir = OpenFileHandle(source, FileAccess.Write, FileShare.Read, FileMode.Open, NativeConstants.FILE_FLAG_BACKUP_SEMANTICS | NativeConstants.FILE_FLAG_OPEN_REPARSE_POINT);
    ...
    public static void CreateDirectoryJunction(string source, ReadOnlyMemory<char> target)
    {

        Directory.CreateDirectory(source);

        using var hdir = OpenFileHandle(source.AsMemory(), FileAccess.Write, FileShare.Read, FileMode.Open, NativeConstants.FILE_FLAG_BACKUP_SEMANTICS | NativeConstants.FILE_FLAG_OPEN_REPARSE_POINT);
    ...
    public static string? QueryDirectoryJunction(SafeFileHandle source)
    {
        ...
        try
        {
            ...
            if (!UnsafeNativeMethods.DeviceIoControl(source, NativeConstants.FSCTL_GET_REPARSE_POINT, IntPtr.Zero, 0U, out buffer[0], (uint)buffer.Length, out var size, IntPtr.Zero))
            {
                return null;
            }
    ...
    /// <param name="source">Handle to directory.</param>
    /// <param name="target">Target path for the junction.</param>
    public static void CreateDirectoryJunction(SafeFileHandle source, ReadOnlyMemory<char> target)
    {

        var namebytes = MemoryMarshal.AsBytes(target.Span);
    ...
    public static IEnumerable<string> QueryDosDevice(string DosDevice)
        => QueryDosDevice(DosDevice.AsMemory());

    public static IEnumerable<string> QueryDosDevice(ReadOnlyMemory<char> DosDevice)
    {
    ...
    public static string GetNtPath(string Win32Path)
        => GetNtPath(Win32Path.AsMemory());

    public static string GetNtPath(ReadOnlyMemory<char> Win32Path)
    {
        var RC = UnsafeNativeMethods.RtlDosPathNameToNtPathName_U(Win32Path.MakeNullTerminated(), out var unicode_string, IntPtr.Zero, IntPtr.Zero);
    ...
    public static void DeleteVolumeMountPoint(ReadOnlyMemory<char> VolumeMountPoint)
        => Win32Try(UnsafeNativeMethods.DeleteVolumeMountPointW(VolumeMountPoint.MakeNullTerminated()));

    public static void SetVolumeMountPoint(ReadOnlyMemory<char> VolumeMountPoint, ReadOnlyMemory<char> VolumeName)
    ...
    public static char FindFirstFreeDriveLetter(char start)
    {
        ...
        for (ushort search = Convert.ToUInt16(start), loopTo = Convert.ToUInt16('Z'); search <= loopTo; search++)
        {
            if ((logical_drives & 1 << search - Convert.ToUInt16('A')) == 0L)
            {
    ...
    public class DriveLayoutInformationType
    {
        ...
        public DRIVE_LAYOUT_INFORMATION_EX DriveLayoutInformation { get; private set; }

        public ReadOnlyCollection<PARTITION_INFORMATION_EX> Partitions { get; private set; }
    ...
    public class DriveLayoutInformationMBR : DriveLayoutInformationType
    {
        ...
        public DRIVE_LAYOUT_INFORMATION_MBR MBR { get; private set; }
    ...
    public class DriveLayoutInformationGPT : DriveLayoutInformationType
    {
        ...
        public DRIVE_LAYOUT_INFORMATION_GPT GPT { get; private set; }
    ...
    public static DriveLayoutInformationType? GetDriveLayoutEx(ReadOnlyMemory<char> DevicePath)
    {
    ...
    public static DriveLayoutInformationType? GetDriveLayoutEx(SafeFileHandle disk)
    {
        ...
        do
        {
            ...
            try
            {
                if (!UnsafeNativeMethods.DeviceIoControl(disk, NativeConstants.IOCTL_DISK_GET_DRIVE_LAYOUT_EX, IntPtr.Zero, 0U, out buffer[0], (uint)buffer.Length, out var arglpBytesReturned, IntPtr.Zero))
                {
                ...
                for (int i = 0, loopTo = layout.PartitionCount - 1; i <= loopTo; i++)
                {
                    ...
                    var prt = buffer.AsSpan(PinnedBuffer<DRIVE_LAYOUT_INFORMATION_EX>.TypeSize + PinnedBuffer<DRIVE_LAYOUT_INFORMATION_GPT>.TypeSize + i * PinnedBuffer<PARTITION_INFORMATION_EX>.TypeSize);

                    partitions[i] = MemoryMarshal.Read<PARTITION_INFORMATION_EX>(buffer.AsSpan(PinnedBuffer<DRIVE_LAYOUT_INFORMATION_EX>.TypeSize + PinnedBuffer<DRIVE_LAYOUT_INFORMATION_GPT>.TypeSize + i * PinnedBuffer<PARTITION_INFORMATION_EX>.TypeSize));
    ...
    private static readonly int _SetDriveLayoutEx_partition_struct_size = PinnedBuffer<PARTITION_INFORMATION_EX>.TypeSize;

    private static readonly int _SetDriveLayoutEx_drive_layout_information_ex_size = PinnedBuffer<DRIVE_LAYOUT_INFORMATION_EX>.TypeSize;

    private static readonly int _SetDriveLayoutEx_drive_layout_information_record_size = PinnedBuffer<DRIVE_LAYOUT_INFORMATION_GPT>.TypeSize;
    ...
    public static void SetDriveLayoutEx(SafeFileHandle disk, DriveLayoutInformationType layout)
    {
        ...
        var size_needed = _SetDriveLayoutEx_drive_layout_information_ex_size
            + _SetDriveLayoutEx_drive_layout_information_record_size
            + partition_count * _SetDriveLayoutEx_partition_struct_size;
        ...
        try
        {
            ...
            pos += _SetDriveLayoutEx_drive_layout_information_ex_size;
            ...
            pos += _SetDriveLayoutEx_drive_layout_information_record_size;

            for (int i = 0, loopTo = partition_count - 1; i <= loopTo; i++)
            {
                var tmp = layout.Partitions;
                var argvalue3 = tmp[i];
                MemoryMarshal.Write(buffer.AsSpan(pos + i * _SetDriveLayoutEx_partition_struct_size), ref argvalue3);
            }
    ...
    public static void FlushBuffers(SafeFileHandle handle) => Win32Try(UnsafeNativeMethods.FlushFileBuffers(handle));

    public static bool? GetDiskOffline(SafeFileHandle disk) => UnsafeNativeMethods.DeviceIoControl(disk, NativeConstants.IOCTL_DISK_GET_DISK_ATTRIBUTES, IntPtr.Zero, 0, out var attribs, PinnedBuffer<GET_DISK_ATTRIBUTES>.TypeSize, out var arglpBytesReturned, IntPtr.Zero)
            ? attribs.Attributes.HasFlag(DiskAttributes.Offline)
            : (bool?)default;

    private static readonly long _TryParseFileTimeUtc_MaxFileTime = DateTime.MaxValue.ToFileTimeUtc();

    public static DateTime? TryParseFileTimeUtc(long filetime)
        => filetime > 0L && filetime <= _TryParseFileTimeUtc_MaxFileTime
    ...
    public static bool? GetDiskReadOnly(SafeFileHandle disk) => UnsafeNativeMethods.DeviceIoControl(disk, NativeConstants.IOCTL_DISK_GET_DISK_ATTRIBUTES, IntPtr.Zero, 0, out var attribs, PinnedBuffer<GET_DISK_ATTRIBUTES>.TypeSize, out var arglpBytesReturned, IntPtr.Zero)
    ...
    public static void SetDiskReadOnly(SafeFileHandle disk, bool read_only)
    {
        ...
        var attribs = new SET_DISK_ATTRIBUTES(flags: DiskAttributesFlags.None, attributesMask: DiskAttributes.ReadOnly, attributes: read_only ? DiskAttributes.ReadOnly : DiskAttributes.None);
    ...
    public static void SetVolumeOffline(SafeFileHandle disk, bool offline)
        => Win32Try(UnsafeNativeMethods.DeviceIoControl(disk, offline ? NativeConstants.IOCTL_VOLUME_OFFLINE : NativeConstants.IOCTL_VOLUME_ONLINE, IntPtr.Zero, 0U, IntPtr.Zero, 0U, out var arglpBytesReturned, IntPtr.Zero));
    ...
    public static IEnumerable<string> EnumerateDiskVolumesMountPoints(ReadOnlyMemory<char> DiskDevice)
    ...
    public static string? GetVolumeNameForVolumeMountPoint(string MountPoint)
        => GetVolumeNameForVolumeMountPoint(MountPoint.AsMemory());

    public static string? GetVolumeNameForVolumeMountPoint(ReadOnlyMemory<char> MountPoint)
    {

        Span<char> str = stackalloc char[50];

        if (UnsafeNativeMethods.GetVolumeNameForVolumeMountPointW(MountPoint.MakeNullTerminated(),
                                                                  out str[0],
                                                                  str.Length)
            && str[0] != '\0')
        {
        ...
        var nt_device_path = QueryDosDevice(ptr).FirstOrDefault();
    ...
    public static string GetVolumePathName(string path) => GetVolumePathName(path.AsMemory());

    public static string GetVolumePathName(ReadOnlyMemory<char> path)
    {
        ...
        try
        {
            return !UnsafeNativeMethods.GetVolumePathNameW(path.MakeNullTerminated(),
                                                                                                                    out result[0],
                                                                                                                    CchBufferLength)
    ...
    public static bool TryGetVolumePathName(ReadOnlyMemory<char> path, out string volume)
    {
        ...
        try
        {
            if (!UnsafeNativeMethods.GetVolumePathNameW(path.MakeNullTerminated(),
                                                                                                                    out result[0],
                                                                                                                    CchBufferLength))
            {
                volume = null!;
    ...
    private static readonly uint _GetScsiAddressAndLength_SizeOfLong = (uint)PinnedBuffer<long>.TypeSize;
    private static readonly uint _GetScsiAddressAndLength_SizeOfScsiAddress = (uint)PinnedBuffer<SCSI_ADDRESS>.TypeSize;
    ...
    public static ScsiAddressAndLength? GetScsiAddressAndLength(ReadOnlyMemory<char> drv)
    {
        ...
        try
        {
            ...
            var rc = UnsafeNativeMethods.DeviceIoControl(disk.SafeFileHandle, NativeConstants.IOCTL_SCSI_GET_ADDRESS, IntPtr.Zero, 0U, out SCSI_ADDRESS ScsiAddress, _GetScsiAddressAndLength_SizeOfScsiAddress, out var arglpBytesReturned, default);
            ...
            long Length, _GetScsiAddressAndLength_SizeOfLong, out var arglpBytesReturned1, default);
    ...
    public static string GetMountPointBasedPath(string path)
        => GetMountPointBasedPath(path.AsMemory());

    public static string GetMountPointBasedPath(ReadOnlyMemory<char> path)
    {

        const string volume_path_prefix = @"\\?\Volume{00000000-0000-0000-0000-000000000000}\";

        if (path.Length > volume_path_prefix.Length && path.Span.StartsWith(@"\\?\Volume{".AsSpan(), StringComparison.OrdinalIgnoreCase))
        {

            var vol = path.Slice(0, volume_path_prefix.Length);
    ...
    public static IEnumerable<string> EnumerateVolumeMountPoints(string VolumeName)
        => EnumerateVolumeMountPoints(VolumeName.AsMemory());

    public static IEnumerable<string> EnumerateVolumeMountPoints(ReadOnlyMemory<char> VolumeName)
    {
        ...
        else if (VolumeName.Span.StartsWith("Volume{".AsSpan(), StringComparison.OrdinalIgnoreCase))
        {
    ...
    public static IEnumerable<string> EnumerateDiskVolumes(string? DevicePath)
        => EnumerateDiskVolumes(DevicePath.AsMemory());

    public static IEnumerable<string> EnumerateDiskVolumes(ReadOnlyMemory<char> DevicePath)
    {

        if (DevicePath.Span.StartsWith(@"\\?\PhysicalDrive".AsSpan(), StringComparison.OrdinalIgnoreCase)
            || DevicePath.Span.StartsWith(@"\\.\PhysicalDrive".AsSpan(), StringComparison.OrdinalIgnoreCase))          // \\?\PhysicalDrive paths to partitioned disks
        {

#if NETCOREAPP || NETSTANDARD2_1_OR_GREATER
            return EnumerateDiskVolumes(uint.Parse(DevicePath.Slice(@"\\?\PhysicalDrive".Length).Span));
#else
            return EnumerateDiskVolumes(uint.Parse(DevicePath.Slice(@"\\?\PhysicalDrive".Length).ToString()));
#endif
        }
        else if (DevicePath.Span.StartsWith(@"\\?\".AsSpan(), StringComparison.Ordinal)
            || DevicePath.Span.StartsWith(@"\\.\".AsSpan(), StringComparison.Ordinal))
        {
    ...
    public static IEnumerable<string> EnumerateVolumeNamesForDeviceObject(string DeviceObject) => DeviceObject.EndsWith("}") &&
            ...
            : new VolumeEnumerator().Where(volumeGuidStr =>
        {
            ...
            try
            {
                ...
                return QueryDosDevice(volumeGuid).Any(target => target.Equals(DeviceObject, StringComparison.OrdinalIgnoreCase));
            }
            catch (Exception ex)
            {
    ...
    public static bool VolumeUsesDisk(ReadOnlyMemory<char> VolumeGuid, uint DiskNumber)
    {
    ...
    public static void ScanForHardwareChanges() => ScanForHardwareChanges(default);

    public static uint ScanForHardwareChanges(ReadOnlyMemory<char> rootid)
    {

        var devInst = 0u;

        var status = UnsafeNativeMethods.CM_Locate_DevNodeW(devInst, rootid.MakeNullTerminated(), 0);
    ...
    public static uint EnumerateDeviceInstancesForService(ReadOnlyMemory<char> service, out IEnumerable<ReadOnlyMemory<char>>? instances)
    {
        instances = null;
        var status = UnsafeNativeMethods.CM_Get_Device_ID_List_SizeW(out var length, service.MakeNullTerminated(), NativeConstants.CM_GETIDLIST_FILTER_SERVICE);
    ...
    public static uint EnumerateDeviceInstancesForSetupClass(ReadOnlyMemory<char> service, out IEnumerable<ReadOnlyMemory<char>>? instances)
    {
        instances = null;
    ...
    public static void RunDLLInstallHinfSection(IntPtr OwnerWindow, string InfPath, ReadOnlyMemory<char> InfSection)
    {
    ...
    public static void InstallFromInfSection(IntPtr OwnerWindow, string InfPath, string InfSection)
    {
        ...
        using var hInf = UnsafeNativeMethods.SetupOpenInfFileW(MemoryMarshal.GetReference(InfPath.AsSpan()),
                                                                                    default,
                                                                                    0x2U,
                                                                                    out var ErrorLine);
    ...
    public static void CreateRootPnPDevice(IntPtr OwnerWindow, string InfPath, ReadOnlyMemory<char> hwid, bool ForceReplaceExistingDrivers, out bool RebootRequired)
    {
        ...
        // '
        var hwIdList = MemoryMarshal.AsBytes($"{hwid}{new char()}{new char()}".AsSpan());
    ...
    public static string? GetPhysicalDeviceObjectNtPath(uint devInst)
    {
        ...
        var rc = UnsafeNativeMethods.CM_Get_DevNode_Registry_PropertyW(devInst, CmDevNodeRegistryProperty.CM_DRP_PHYSICAL_DEVICE_OBJECT_NAME, out var regtype, out buffer[0], buffersize, 0);
    ...
    public static IEnumerable<string>? GetDeviceRegistryProperty(uint devInst, CmDevNodeRegistryProperty prop)
    {
        ...
        var rc = UnsafeNativeMethods.CM_Get_DevNode_Registry_PropertyW(devInst, prop, out var regtype, out buffer[0], buffersize, 0);
    ...
    public static IEnumerable<string> EnumerateWin32DevicePaths(ReadOnlyMemory<char> nt_device_path)
    {

        var query = from dosdevice in QueryDosDevice()
                    where QueryDosDevice(dosdevice).Any(target => target.AsSpan().Equals(nt_device_path.Span, StringComparison.OrdinalIgnoreCase))
                    select dosdevice;
    ...
    public static IEnumerable<string> EnumerateRegisteredFilters(uint devInst)
    {
        ...
        try
        {
            var rc = UnsafeNativeMethods.CM_Get_DevNode_Registry_PropertyW(devInst, CmDevNodeRegistryProperty.CM_DRP_UPPERFILTERS, out var regtype, out buffer[0], buffersize, 0);
    ...
    public static int RemovePnPDevice(IntPtr OwnerWindow, ReadOnlyMemory<char> hwid)
    {
    ...
    public static void UpdateDriverForPnPDevices(IntPtr OwnerWindow, string InfPath, ReadOnlyMemory<char> hwid, bool forceReplaceExisting, out bool RebootRequired)
    {
    ...
    public static void DriverPackagePreinstall(string InfPath)
    {
        ...
        var errcode = UnsafeNativeMethods.DriverPackagePreinstallW(MemoryMarshal.GetReference(InfPath.AsSpan()), 1);
    ...
    public static void DriverPackageInstall(string InfPath, out bool NeedReboot)
    {
        ...
        var errcode = UnsafeNativeMethods.DriverPackageInstallW(MemoryMarshal.GetReference(InfPath.AsSpan()), 1, default, out NeedReboot);
    ...
    public static void DriverPackageUninstall(string InfPath, DriverPackageUninstallFlags Flags, out bool NeedReboot)
    {
        ...
        var errcode = UnsafeNativeMethods.DriverPackageUninstallW(MemoryMarshal.GetReference(InfPath.AsSpan()), Flags, default, out NeedReboot);
    ...
    public static bool MapFileAndCheckSum(ReadOnlyMemory<char> file, out int headerSum, out int checkSum)
        => UnsafeNativeMethods.MapFileAndCheckSumW(file.MakeNullTerminated(), out headerSum, out checkSum) == 0;
    ...
    public static void UpdateDiskProperties()
    {
                                   ...
                                   select device)
        {
            ...
            try
            {
                using var device = OpenFileHandle($@"\\?\{diskdevice}".AsMemory(), 0, FileShare.ReadWrite, FileMode.Open, Overlapped: false);
    ...
    public static bool UpdateDiskProperties(SafeFileHandle devicehandle, bool throwOnFailure)
    {
        ...
        var rc = UnsafeNativeMethods.DeviceIoControl(devicehandle, NativeConstants.IOCTL_DISK_UPDATE_PROPERTIES, IntPtr.Zero, 0U, IntPtr.Zero, 0U, out var arglpBytesReturned, IntPtr.Zero);
    ...
    /// </summary>
    /// <returns>Returns a value indicating whether operation was successful or not.</returns>
    public static bool UpdateDiskProperties(ReadOnlyMemory<char> DevicePath)
    {
    ...
    /// </summary>
    public static KeyValuePair<ReadOnlyMemory<char>, SafeFileHandle> OpenDiskByScsiAddress(SCSI_ADDRESS ScsiAddress, FileAccess AccessMode)
    {
        ...
        KeyValuePair<ReadOnlyMemory<char>, SafeFileHandle> filter(string diskdevice)
        {
            ...
            try
            {
                var devicehandle = OpenFileHandle(diskdevice.AsMemory(), AccessMode, FileShare.ReadWrite, FileMode.Open, Overlapped: false);
                ...
                try
                {
                    ...
                    return new KeyValuePair<ReadOnlyMemory<char>, SafeFileHandle>(diskdevice.AsMemory(), devicehandle);
                }
        ...
        var dev = (from anydevice in rawdevices.Concat(volumedevices)
                   select filter(anydevice)).FirstOrDefault(anydevice => !anydevice.Key.IsEmpty);

        return dev.Key.IsEmpty
    ...
    public static string? GetDeviceNameByScsiAddressAndSize(SCSI_ADDRESS scsi_address, long disk_size)
    {
        ...
        bool filter(string diskdevicestr)
        {
            ...
            var diskdevice = $@"\\?\{diskdevicestr}".AsMemory();
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (!got_address.HasValue || !got_address.Value.Equals(scsi_address))
                    {
                    ...
                    if (disk_size is var arg5 && got_size is { } arg6 && arg5 == arg6)
                    {
                        return true;
                    }
    ...
    public static bool TestFileOpen(ReadOnlyMemory<char> path)
    {
        using var handle = UnsafeNativeMethods.CreateFileW(path.MakeNullTerminated(),
    ...
    public static bool TestFileOpen(string path) => TestFileOpen(path.AsMemory());

    public static void CreateHardLink(ReadOnlyMemory<char> existing, ReadOnlyMemory<char> newlink)
        => Win32Try(UnsafeNativeMethods.CreateHardLinkW(newlink.MakeNullTerminated(), existing.MakeNullTerminated(), IntPtr.Zero));

    public static void MoveFile(ReadOnlyMemory<char> existing, ReadOnlyMemory<char> newname)
    ...
    public struct ByHandleFileInformation
    {
        ...
        public static ByHandleFileInformation FromHandle(SafeFileHandle handle)
        {
            ...
            var obj = new ByHandleFileInformation();

            Win32Try(UnsafeNativeMethods.GetFileInformationByHandle(handle, out obj));
