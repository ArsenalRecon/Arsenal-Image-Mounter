...
#Enable Warning CA1401 ' P/Invokes should not be visible
        ...
        Public NotInheritable Class UnsafeNativeMethods
            ...
            Friend Declare Unicode Function FindNextStreamW Lib "kernel32" Alias "FindNextStreamW" (
            ...
            Friend Declare Unicode Function StartService Lib "advapi32" (
            ...
            Friend Declare Unicode Function FreeLibraryW Lib "kernel32" (
            ...
            Friend Declare Unicode Function SetupDiEnumDeviceInfoW Lib "setupapi" (
            ...
            Friend Declare Unicode Function SetupDiRestartDevicesW Lib "setupapi" (
              DeviceInfoSet As SafeDeviceInfoSetHandle,
            ...
            Friend Declare Unicode Function SetupDiEnumDeviceInterfacesW Lib "setupapi" (
              DeviceInfoSet As SafeDeviceInfoSetHandle,
            ...
            Friend Declare Unicode Function SetupDiCreateDeviceInfoListW Lib "setupapi" (
              <[In], IsReadOnly> ByRef ClassGuid As Guid,
            ...
            Friend Declare Unicode Function SetupDiCallClassInstallerW Lib "setupapi" (
...
{in_use_apps(0)}", ex)
        ...
        Public Shared Async Function GetFileVersionAsync(exe As Stream, cancel As CancellationToken) As Task(Of Version)
...
#If NETSTANDARD2_1_OR_GREATER OrElse NETCOREAPP Then
                    Dim length = Await exe.ReadAsync(buffer.AsMemory(0, CInt(exe.Length)), cancel).ConfigureAwait(False)
#Else
                    Dim length = Await exe.ReadAsync(buffer, 0, CInt(exe.Length), cancel).ConfigureAwait(False)
#End If
                    Return NativePE.GetFixedFileVerInfo(buffer.AsSpan(0, length)).FileVersion
        ...
        Public Shared Function GetFileVersion(exepath As String) As Version
            ...
            Using exe As New FileStream(exepath, FileMode.Open, FileAccess.Read, FileShare.Read Or FileShare.Delete, bufferSize:=4096, useAsync:=True)
                ...
                Return GetFileVersion(exe)

            End Using

        End Function

        Public Shared Async Function GetFileVersionAsync(exepath As String, cancel As CancellationToken) As Task(Of Version)

            Using exe As New FileStream(exepath, FileMode.Open, FileAccess.Read, FileShare.Read Or FileShare.Delete, bufferSize:=4096, useAsync:=True)

                Return Await GetFileVersionAsync(exe, cancel).ConfigureAwait(False)
        ...
        Public Shared Function GetExeFileHeader(exe As Stream) As IMAGE_NT_HEADERS
            ...
            Dim buffer = ArrayPool(Of Byte).Shared.Rent(CInt(exe.Length))
            Try
                exe.Read(buffer, 0, CInt(exe.Length))
        ...
        Public Shared Function GetExeFileHeader(exe As ReadOnlyMemory(Of Byte)) As IMAGE_NT_HEADERS

            Return NativePE.GetImageNtHeaders(exe.Span)
...
#End If
        ...
        Public Shared Sub RestartDevice(devclass As Guid, devinst As UInteger)
            ...
            Using devinfo = UnsafeNativeMethods.SetupDiGetClassDevsW(devclass,
                ...
                While UnsafeNativeMethods.SetupDiEnumDeviceInfoW(devinfo, deviceIndex, devInfoData)
                    ...
                    If devInfoData.DevInst.Equals(devinst) Then
                        ...
                        If UnsafeNativeMethods.SetupDiSetClassInstallParamsW(devinfo, devInfoData, pcp, PinnedBuffer(Of SP_PROPCHANGE_PARAMS).TypeSize) AndAlso
                            UnsafeNativeMethods.SetupDiCallClassInstallerW(NativeConstants.DIF_PROPERTYCHANGE, devinfo, devInfoData) Then
        ...
        Public Shared Sub CreateRootPnPDevice(OwnerWindow As IntPtr, InfPath As String, hwid As ReadOnlyMemory(Of Char), ForceReplaceExistingDrivers As Boolean, <Out> ByRef RebootRequired As Boolean)
            ...
            ''
            Dim DeviceInfoSet = UnsafeNativeMethods.SetupDiCreateDeviceInfoListW(ClassGUID, OwnerWindow)
            ...
            Using DeviceInfoSet
                ...
                '' in the PnP HW tree.
                ''
                Win32Try(UnsafeNativeMethods.SetupDiCallClassInstallerW(DIF_REGISTERDEVICE,
                                                                        DeviceInfoSet,
                                                                        DeviceInfoData))
...
#End If
        ...
        Public Shared Function RemovePnPDevice(OwnerWindow As IntPtr, hwid As ReadOnlyMemory(Of Char)) As Integer
            ...
            '' Create the container for the to-be-created Device Information Element.
            ''
            Dim DeviceInfoSet = UnsafeNativeMethods.SetupDiCreateDeviceInfoListW(IntPtr.Zero,
                                                                 OwnerWindow)
            If DeviceInfoSet.IsInvalid Then
            ...
            Using DeviceInfoSet
                ...
                Dim done As Integer
                Do
                    If Not UnsafeNativeMethods.SetupDiEnumDeviceInfoW(DeviceInfoSet, i, DeviceInfoData) Then
                        If i = 0 Then
                    ...
                    If UnsafeNativeMethods.SetupDiCallClassInstallerW(DIF_REMOVE, DeviceInfoSet, DeviceInfoData) Then
                        done += 1
                    End If
